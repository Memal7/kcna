# Kubernetes Objects
This part of the repository demonstrates the **Kubernetes Objects** which is the part of [Kubernetes Fundamentals - 46%](./03_kubernetes-fundamentals.md).

---

## Chapter outcomes
- Describe the Kubernetes Objects and understand the task of every single object in much detail and in-depth.
...
- Some core primitives K8s objects are:
    ...
    - **Controller:** keeps the whole cluster in the desired state (e.g. Deployments, ReplicaSet, etc.).
...
- Objects can be described in YAML file and then send to API Server, which is going to be validated before they're created.
- An example k8s object declaration in YAML file looks like following:
```
...
- All communications between all k8s components are done through API Server and this is where users would access the cluster.
...
- Before a request is processed by k8s, it has to go through three stages:
    - **Authentication:** The requester needs to present a means of identity to authenticate against the API. Commonly done with a digitally signed certificate (X.509) or with an external identity management system. K8s users are always externally managed. Service Accounts can be used to authenticate technical users
...
- **Pods:**
    ...
    - Pods are ephemeral - no Pod is ever re-deployed. If a Pod dies and re-deploys again based on a container image, no state is maintained between those deployments.
...
- **Controller:**
    ...
    - However, you don't have to manage each Pod. To make sure that a defined number of Pod copies runs all the time, you can use Controller objects (e.g. Deployment, ReplicaSet, DaemonsSet, etc.) that make sure the right number of the right kind of Pod is always running, to match the desired state you specified.
- **ReplicaSet:**
    - ReplicaSet ensures a desired number of Pods are running at any given time.
    - Define the number of replicas for a particular Pod and make sure if one of the Pods becomes unavailable or unhealthy that you want to have it up and running at all times.
...
- **DaemonSet:**
    ...
    - DaemonSets is perfect to run the infrastructure-related workload, e.g. monitoring or logging tools.
- **Job:**
	- Job object creates one or more Pods that execute a task and terminate afterward.
...
- **Describe the concept of persistent storage in k8s:**
    ...
    - But a cluster environment with multiple servers requires even more flexibility. So in this case, you can use cloud block storage like Amazon EBS, Google Persistent Disks, and Azure Disk Storage.
	...
	- To use this abstraction, you have two more objects that can be used:
		- **PersistentVolumes (PV):** It's a Pod-independent storage that's defined by admin at the cluster level. The object configuration holds information like the type of volume, volume size, access mode, unique identifiers, and information on how to mount it.
...
### An overview of Kubernetes Storage objects with a short description of every component.
...
- **Horizontal Pod Autoscaler (HPA):**
    ...
    - HPA scaling calculations can also use custom or external metrics. Custom metrics target a marker of Pod usage other than CPU usages, such as network traffic, memory, or a value relating to the Pod’s application. External metrics measure values that do not correlate to a Pod. For example, an external metric could track the number of pending tasks in a queue.
- **Cluster Autoscaler:**
...
[Next Part ▶ 04_Cloud Native Application Delivery](./04_cloud-native-app-delivery.md)
